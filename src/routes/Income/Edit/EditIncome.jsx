...
import './style.css';
import { GetCurrentUserUID, GetOneIncomeData, UpdateIncomeDataToFirestore } from '../../../lib/firebase/auth';
...
const EditIncome = () => {
  ...
  const { fetchTotalExpense, fetchTotalIncome, fetchIncome } = useAuth();
  ...
  let uqId = location.pathname.split("/all-income/edit/");
  uqId = uqId[1];
  useEffect(() => {
  ...
  const addingExpenseData = async (e) => {
    ...
    if (expenseCategory === '') {
      ...
      try {
        let addingData = await UpdateIncomeDataToFirestore(index, Uid, note, amount, expenseCategory, date, expenseAccount, noteDescription, currentDate);
          ...
          <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 l:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form className='flex flex-col gap-4' action='#' onSubmit={addingExpenseData}>
                ...
                <div className='flex gap-4 xxl:flex-wrap'>
