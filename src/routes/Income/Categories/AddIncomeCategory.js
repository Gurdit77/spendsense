...
import { GetCurrentUserUID, UpdateExpenseCategory, AddIncomeCategories } from '../../../lib/firebase/auth';
...
function AddIncomeCategory() {  
  const {getPageTitle} = useThemeConfig();  
    ...
    const {userLoggedIn, incomeCategories, setIncomeCategories} = useAuth();
  ...
  useEffect(() => {   
      ...
      const addingExpenseData = async (e)=>{
        ...
        if (newName === '') {     
            ...
            const hi = await AddIncomeCategories(Uid,newName);            
            if(hi === "success"){
                setErrorMessage('');
                setNewName('');
  ...
  return (
    ...
    {!userLoggedIn && (<Navigate to={'/'} />)}
      ...
      <CategoriesTabs  active={true} page="Income" />
...
<Heading headingStyle="h2" text="Income Categories" />
<ul className='grid sm:grid-cols-2 gap-x-3'>
      {Object.entries(incomeCategories).map(([key, val], index) => (
